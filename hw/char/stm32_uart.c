...
#define STM32_UART(obj) \
    OBJECT_CHECK(Stm32Uart, (obj), "stm32_uart")
...
static int stm32_uart_init(SysBusDevice *dev)
{
    ...
    s->rx_timer =
          timer_new_ns(QEMU_CLOCK_VIRTUAL, stm32_uart_rx_timer_expire, s);
    s->tx_timer =
          timer_new_ns(QEMU_CLOCK_VIRTUAL, stm32_uart_tx_timer_expire, s);
